...
class Map extends React.Component {
    ...
    componentDidMount() {
        let width = Math.floor(window.innerWidth * 105 / 100);
        let height = Math.floor(window.innerHeight * 99 / 100);
        svg = d3.select("#map").append("svg").attr("width", width).attr("height", height).call(d3.zoom().scaleExtent([0.1, 10]).on("zoom", function () {
            svg.attr("transform", d3.event.transform)
        }))
            .append("g");
        
        svg.append("image").attr("xlink:href", "bulgaria2.svg").attr("width", width).attr("height", height);
